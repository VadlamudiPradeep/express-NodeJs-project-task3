...
function displayList(e) {
    ...
    let getRequest=async()=>await axios({
        ...
        if (res.data.response==0 || !res.data.response){
            ...
            let amountItem=document.createTextNode(expenseDetails.amount>0?`+₹${(expenseDetails.amount*1)
                .toLocaleString('en-IN')}`:`-₹${(expenseDetails.amount*-1).toLocaleString('en-IN')}`)
            ...
            delBtn.innerHTML=`<i id='${expenseDetails.id}' style='font-size:22px'</i>`
